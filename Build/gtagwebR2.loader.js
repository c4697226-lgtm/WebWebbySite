function createUnityInstance(r, n, l) {
    ...
    function b(n) {
        ...
        return e(c[n], {
                        ...
                        for (var n in m) {
                            ...
                            if (m[n].hasUnityMarker(i))
                                return s && console.log('You can reduce startup time if you configure your web server to add "Content-Encoding: ' + n + '" response header when serving "' + s + '" file.'),
                                    (t = m[n]).worker || (o = URL.createObjectURL(new Blob(["this.require = ", t.require.toString(), "; this.decompress = ", t.decompress.toString(), "; this.onmessage = ", function (e) {
                                        e = {
                                            id: e.data.id,
                                    ...
                                    a = t.worker.nextCallbackId++,
                                    t.worker.callbacks[a] = e,
            ...
            "file:" == location.protocol ? d(r + ". Loading web pages via a file:// URL without a web server is not supported by this browser. Please use a local development web server to host Unity content, or use the Unity Build and Run option.", "error") : console.error(r)
        })
    }
    function w() {
                            ...
                            for (e in r) {
                                ...
                                if (c.frameworkUrl.endsWith("." + t[0]))
                                    return n = "Unable to parse " + c.frameworkUrl + "!",
                                        "file:" == location.protocol ? void d(n + " Loading pre-compressed (brotli or gzip) content via a file:// URL without a web server is not supported by this browser. Please use a local development web server to host compressed Unity content, or use the Unity Build and Run option.", "error") : (n += ' This can happen if build compression was enabled but web server hosting the content was misconfigured to not serve the file with HTTP Response Header "Content-Encoding: ' + t[1] + '" present. Check browser Console and Devtools Network tab to debug.',
    ...
    return new Promise(function (e, r) {
        ...
        c.SystemInfo.hasWebGL ? 1 == c.SystemInfo.hasWebGL ? (n = 'Your browser does not support graphics API "WebGL 2" which is required for this content.',
            ...
            r(n)) : c.SystemInfo.hasWasm ? (c.startupErrorHandler = r,
                ...
                w()) : r("Your browser does not support WebAssembly.") : r("Your browser does not support WebGL.")
    }
    )
}
